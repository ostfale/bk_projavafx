...
/**
 * The role of a presenter is mainly to get the user’s inputs from the view and act upon them. The presenter will
 * have a reference to the view. It will add event listeners to the view, so it is notified when the data in the
 * iew change. In the event handlers, the presenter takes control and processes the inputs. If the application requires
 * a reference to the presenter in the view, you can have that as an argument to the constructor of the view class.
 * Alternatively, you can provide a setter method in the view class to set the presenter.
 * Created :  20.05.2020
 *
